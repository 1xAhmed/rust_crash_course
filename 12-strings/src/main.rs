...
// There are atleast SIX types of strings in rust standard library, but we mostly care about TWO of them to overlap each other
        ...
        // In other words, a borrowed string slice is a subset of strings in more ways then one. Which is why they share a bunch of other characterisitics. For example, both string types are valid UTF-8, by defination, by compiler enforecement and by runtime checks.
        ...
        // You can use the bytes method `word.bytes();` to access the vector of UTF-8 bytes, which you can index into it if you want, since bytes are fixed size. This woks fine for simple English text, as long you stick to the portion of ASCII. You can use the chars method `word.chars();` to retreive an iterator that you can use to iterate through unicode scalers. 

        // If you end up using manually using one of the iterators. Iterators have a handy method called `.nth(2)` That you can use in the place of indexing
